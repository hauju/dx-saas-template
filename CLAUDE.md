...
- **`src/passkeys.rs`** — Dual-target: `list_passkeys` / `delete_passkey` server functions over `user_passkeys` and the Settings `PasskeysCard` (list, remove, labelled enrollment via dx-auth's `enroll_passkey_named`). Rendered only in local auth mode.
...
- **`polar`** — Polar.sh billing API: customers, subscriptions, checkout, orders, webhook verification. Not yet extracted to dx-kit (its product-ID config fields are still app-specific).
...
- **Client auth state**: `UserAuthState` is a `Memo` derived in `App` from the `use_server_future` that fetches `/api/me`, provided via context; a `UserDataRefreshTrigger` signal re-fetches on demand. Derived, not copied by an effect, because effects never run on the server: the server function sees the request's session during SSR, so dashboard pages render their real content on the first pass and the client hydrates with the same value. Keep it that way for any state a page needs at first paint (`SiteFlags` works the same).
...
- **OAuth-for-MCP**: A self-hosted OAuth 2.1 authorization server (`src/server/oauth/`) — protected-resource + AS metadata (RFC 9728/8414), dynamic client registration (RFC 7591, redirect-URI allowlist is the security boundary), auth-code + PKCE S256 reusing the login session, and a token endpoint that mints `oat_` tokens bound to the client and its granted scope, expiring after 30 days (`oauth::token::ACCESS_TOKEN_TTL_SECONDS`; no refresh token, MCP clients re-run the flow on 401). `api_key::verify` refuses expired keys; the MCP server refuses client tokens without the `mcp` scope (`ApiAuth::allows`). The MCP endpoint (`POST /mcp`, `src/server/mcp.rs`) is an `rmcp` 3.x `StreamableHttpService` (MCP spec `2026-07-28`, negotiating down to `2024-11-05`) with tools defined via `#[tool_router]`/`#[tool]` on `McpTools`; tools authenticate via the shared `api_auth::authenticate`. An `mcp_auth_challenge` middleware does a presence-only check — no credential returns `401` + `WWW-Authenticate` pointing at the metadata (triggering OAuth discovery). Add new tools as `#[tool]` methods.
- **MCP handler wiring has two non-obvious requirements** (`src/server/mcp.rs`): `#[tool_handler(router = self.tool_router)]` generates `call_tool`/`list_tools`/`get_tool` — pass `router` explicitly, because the attribute otherwise defaults to `Self::tool_router()` and rebuilds the router, re-deriving every tool's JSON schema, on each call. And leave `initialize` to its default: it negotiates the protocol version against the client's request, which a hand-written override silently pins to the server's newest, breaking older clients (and, per SEP-2567, deciding whether the request is served statelessly).
- **MCP `Host` validation**: rmcp validates the `Host` header to block DNS rebinding, and its default allowlist is **loopback-only** — a deployed instance would `403` every MCP request. `allowed_hosts()` derives the list from `BASE_URL` (bare hostname, so it matches any port behind a proxy) and keeps loopback for local development and container health checks. Covered by `mcp_accepts_the_host_from_base_url`.
- **Billing webhooks**: `POST /webhooks/polar` (`src/server/billing.rs`) verifies the Standard Webhooks signature and syncs `SubscriptionInfo` onto the user. Deliveries are applied exactly once (the `webhook-id` is claimed in `polar_webhook_events` inside the same transaction) and never backwards (the payload's `modified_at` is stored as `updated_at` and an older event does not overwrite a newer one). Gate premium features with `billing::require_active(&user.subscription)?` (maps to `402`).
