...
The template uses **[FerrisKey](https://ferriskey.dev)** as the identity provider, integrated via OIDC (authorization code flow with PKCE) and a custom login UI that drives FerrisKey's REST API directly. Authentication is session-based using `tower-sessions` with PostgreSQL as the session store.
...
<Steps>
  ...
  <Step title="Branch on credentials">
    ...
    - **No credentials / new user** → email-OTP flow, gated by the registration policy and (when configured) a Bollwark captcha: a 6-digit code is sent via SMTP and verified at `/auth/session/otp/verify`.
  </Step>
  ...
  <Step title="Session cookie is set">
    The server records the user via `AuthUserStore::lookup_or_create_user` and persists the session in PostgreSQL. The signed cookie is HTTP-only and (in production) secure + SameSite=Lax.
  </Step>
...
New accounts are created by the email-OTP flow, and who may create one is a deployment setting:
...
| `CAPTCHA_URL` / `CAPTCHA_SITE_KEY` / `CAPTCHA_SECRET_KEY` | A [Bollwark](https://github.com/hauju/bollwark) captcha in front of registration. The login page mounts the widget and the server verifies its token; all three must be set. |
